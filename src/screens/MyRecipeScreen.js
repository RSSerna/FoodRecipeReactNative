...
export default function MyRecipeScreen() {
    ...
    const handlerecipeClick = (recipe) => {
        navigation.navigate("CustomNewsScreen", { recipe });
    };

    const deleteRecipe = async (index) => {
        try {
            const updatedArticles = [...recipes];
            updatedArticles.splice(index, 1); // Remove article from array
            await AsyncStorage.setItem("customRecipes", JSON.stringify(updatedArticles)); // Update AsyncStorage
            setRecipes(updatedArticles); // Update state
        } catch (error) {
